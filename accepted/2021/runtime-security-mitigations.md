...
Enclaves protect a portion of memory. It is implemented by Intel Software Guard Extensions (SGX). To fit into Enclaves programming model, the software stack typically requires refactoring of sensitive computation into isolated component that runs inside the Enclave and [uses specialized APIs for communication](https://openenclave.io/sdk/). The current enclave capabilities are too limitted to fully suport a general purpose .NET runtime. .NET runtime for enclaves would be a niche [form factor](https://github.com/dotnet/designs/blob/main/accepted/2020/form-factors.md) with number of limitations.
