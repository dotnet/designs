...
* Users can install and interact with workloads for an SDK via any supported installation tool, such as the VS Installer, a Linux package manager, or the CLI.
...
* *Composition* describes file formats, pack types, on-disk locations, and other mechanisms that define a workload, constituent parts, and their associated versions, that enable them to be acquired, updated, and used as a coherent set.
...
Note: The term "supported installation tool" is intended to mean an installation system that exposes components and manages their dependencies. For example, Visual Studio Installer, Docker, and APT do that, while MSI and PKG don't.
...
Users should be able to install workloads via an install tool without any sense that there is a compositional model at play.
...
* .NET 6.0+
  * Make WPF, Windows Forms and ASP.NET Core as workloads
  ...
  * Define interaction with installation tools.
