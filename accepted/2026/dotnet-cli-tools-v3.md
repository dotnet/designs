...
The key take-away is that runtime resolution occurs at publish time for NAOT, not as part of package acquisition. That means that we don't need TFM as a key bit of design currency in the design. The primary problem is that tools pointer package acts as a runtime gate when that's not needed; it is actively harmful. It's also challenging to coordinate runtime gates for NAOT (not needed) and `any` fallback packages (needed).
