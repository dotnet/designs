...
We are updating the dotnet-trace tool so that it can be a CLI front-end for both the OS-neutral and OS-integrated profiling approaches. In particular for anyone using a Linux kernel supporting user_events (6.4+) and .NET 10 apps we expect the new capabilities in dotnet-trace will offer a superior experience to PerfCollect. We are expecting:
