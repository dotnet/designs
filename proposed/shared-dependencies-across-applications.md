...
- Each `sharedDependencies` entry is an absolute path to the shared set's `.deps.json`. We can relax this later if there is a specific need (for example, paths relative to the app or environment variables).
